...
- **Rendering**:
  ...
  - Deeper-layer centers are projected from the active layer center into each rendered layer, so child hexes keep stable positions relative to their parent active-layer hexes.
  ...
  - World hex rendering is clipped to the active visible field disk; perimeter dot markers are intentionally not clipped and may overlap the border line.
...
- Session restore no longer requires the legacy `hexigame.guest.started` gate: if a valid persisted session snapshot exists, resume is available and game state can be restored.
- Session reload contract: when a valid persisted session exists, application boot reconnects directly into gameplay instead of returning to the start screen.
...
- Audio seek contract: saved playback time is applied even if metadata is not yet loaded at restore time; the driver defers `seek` until metadata becomes available.
